...
function completeMultipartUpload(authInfo, request, log, callback) {
    ...
    return async.waterfall([
        ...
        function ifAWSBackend(destBucket, objMD, mpuBucket, next) {
            if (config.locationConstraints[objectLocationConstraint].type ===
            'aws_s3') {
                ...
                uploadId, log, (err, data) => {
                    ...
                    return next(skipError, destBucket, data.ETag,
                    data.VersionId);
                });
            }
            return next(null, destBucket, objMD, mpuBucket);
