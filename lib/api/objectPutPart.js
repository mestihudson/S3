...
import multipleBackendGateway from '../data/multipleBackendGateway';
...
export default function objectPutPart(authInfo, request, streamingV4Params, log,
    cb) {
    console.log('\n\nREQUEST QUERRYYYYYYY: ', request.query);
    console.log('\nBUCKETNAMEMMEMEMEM: ', request.bucketName);
    ...
    return async.waterfall([
        ...
        (destinationBucket, objectLocationConstraint, cipherBundle,
            ...
            if (config.backends.data === 'multiple') {
                return multipleBackendGateway.uploadPart(objectKey, uploadId,
                objectLocationConstraint, objectKeyContext.partNumber,
                (err, data) => {
                    if (err) {
                        return next(err, destinationBucket);
                    }
                    return next(null, destinationBucket, data, null,
