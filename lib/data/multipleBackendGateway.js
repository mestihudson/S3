...
const multipleBackendGateway = {
    ...
    createMPU: (key, websiteRedirectHeader, location, cb) => {
        const client = clients[location];
        if (client.clientType === 'aws_s3') {
            const params = { Bucket: client.awsBucketName, Key: key,
                WebsiteRedirectLocation: websiteRedirectHeader };
            client.createMultipartUpload(params, cb);
        } else {
            const data = {};
            data.UploadId = UUID.v4().replace(/-/g, '');
            cb(null, data);
        }
    },

    uploadPart: (location, key, uploadId, partNumber, log, cb) => {
        // client should always be AWS
        const client = clients[location];
        const params = { Bucket: client.awsBucketName, Key: key,
    ...
    completeMPU: (key, uploadId, log, cb) => {
        // client should always be AWS
        const client = clients['aws-test'];
        const bucket = client.awsBucketName;
        const params = { Bucket: bucket, Key: key,
            UploadId: uploadId };
        client.listParts(params, (err, data) => {
            if (err) {
                log.error('err from data backend',
                { err, dataStoreName: client.dataStoreName });
                return cb(errors.InternalError);
            ...
            const partArray = [];
            return async.each(data, (partObj, callback) => {
                const partParams = { PartNumber: partObj.PartNumber,
                ETag: partObj.ETag };
                ...
                return client.completeMultipartUpload(mpuParams, (err, data) => {
