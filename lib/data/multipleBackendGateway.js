...
const multipleBackendGateway = {
    uploadPart: (client, key, uploadId, partNumString, reqUids, cb) => {
        const partNumber = Number(partNumString);
    ...
    put: (isPart, stream, size, keyContext, backendInfo,
    reqUids, callback) => {
        ...
        if (client.clientType === 'aws_s3') {
            ...
            if (isPart) {
                return multipleBackendGateway.uploadPart(client,
                keyContext.objectKey, uploadId, partNumber, reqUids, callback);
            }
